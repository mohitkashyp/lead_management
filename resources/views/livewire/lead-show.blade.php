                        ...
                        <div class="px-4 py-5 sm:p-6">
                            ...
                            <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                                ...
                                <div>
                                    ...
                                    <dd class="mt-1 text-sm {{ $lead->next_follow_up_date->isPast() ? 'text-red-600 font-medium' : 'text-gray-900' }}">
                    ...
                    <div class="bg-white shadow rounded-lg">
                        <div class="px-4 py-5 sm:p-6">
                            <div class="flex items-center justify-between mb-4">
                                ...
                                <button wire:click="toggleActivityForm" type="button" class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700">
                                    <svg class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <div class="grid grid-cols-1 gap-4">
                                    <div class="grid grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            <select wire:model="activity_type" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                        ...
                                        <div>
                                            ...
                                            <input type="datetime-local" wire:model="activity_date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                        </div>
                                    </div>

                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Subject</label>
                                        <input type="text" wire:model="subject" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm @error('subject') border-red-500 @enderror">
                                    ...
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700">Next Follow-up Date (Optional)</label>
                                        <input type="date" wire:model="next_follow_up_date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                    </div>

                                    <div class="flex justify-end gap-3">
                                        <button type="button" wire:click="toggleActivityForm" class="px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50">
                                            Cancel
                                        </button>
                                        <button type="button" wire:click="saveActivity" class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700">
                                            Save Activity
                                        </button>
                                    </div>
                                </div>
                            ...
                            <div class="mb-4 flex gap-2">
                                <button wire:click="quickAddCall" type="button" class="inline-flex items-center px-3 py-1.5 border border-gray-300 text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50">
                                    <svg class="w-3 h-3 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                                    </svg>
                                    Log Call
                                </button>
                                <button wire:click="quickAddEmail" type="button" class="inline-flex items-center px-3 py-1.5 border border-gray-300 text-xs font-medium rounded text-gray-700 bg-white hover:bg-gray-50">
                                    <svg class="w-3 h-3 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            ...
                                            <div class="relative flex space-x-3">
                                                ...
                                                <div class="min-w-0 flex-1 pt-1.5 flex justify-between space-x-4">
                                                    ...
                                                    <div class="text-right text-sm whitespace-nowrap text-gray-500">
                                                        <time datetime="{{ $activity->activity_date }}">{{ $activity->activity_date->format('M d, Y') }}</time>
                                                        <p class="text-xs">{{ $activity->activity_date->format('h:i A') }}</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                ...
                <div class="lg:col-span-1 space-y-6">
                    ...
                    <div class="bg-white shadow rounded-lg">
                        <div class="px-4 py-5 sm:p-6">
                            ...
                            <div class="space-y-3">
                                @if(!$lead->customer_id)
                                <button wire:click="convertToCustomer" type="button" class="w-full inline-flex justify-center items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-green-600 hover:bg-green-700">
                                    <svg class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <a href="{{ route('orders.create', $lead->id) }}" class="w-full inline-flex justify-center items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700">
                                    <svg class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <a href="{{ route('leads.edit', $lead->id) }}" class="w-full inline-flex justify-center items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                                    <svg class="w-4 h-4 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                    Edit Lead
                    ...
                    <div class="bg-white shadow rounded-lg">
                        <div class="px-4 py-5 sm:p-6">
                            ...
                            <div class="space-y-3">
                                <select wire:model="new_status_id" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                    @foreach($statuses as $status)
                                        <option value="{{ $status->id }}">{{ $status->display_name }}</option>
                                    @endforeach
                                </select>

                                <button wire:click="updateStatus" type="button" class="w-full inline-flex justify-center items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700">
                                    Update Status
                                </button>
                            </div>
                        </div>
                        ...
                        <div class="px-4 py-5 sm:p-6">
                            ...
                            <dl class="space-y-3">
                                ...
                                <div>
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900">{{ $lead->updated_at->diffForHumans() }}</dd>
                                </div>

                                @if($lead->converted_at)
                                <div>
                                    <dt class="text-xs font-medium text-gray-500">Converted On</dt>
                                    <dd class="mt-1 text-sm text-gray-900">{{ $lead->converted_at->format('M d, Y') }}</dd>
                                </div>
                                @endif
                            </dl>
                        </div>
